...
def add_lumi():
    ...
    lumi.AddText("35.9-138 fb^{-1} (13 TeV)")
    return lumi
...
def add_Preliminary():
    lowX=0.21
    lowY=0.86
    lumi  = ROOT.TPaveText(lowX, lowY+0.05, lowX+0.15, lowY+0.16, "NDC")
...
if __name__ == "__main__":
    ...
    list_BRpp=array("d",array_BRpp)

    # HIG-22-007
    x_llbb_obs, y_llbb_obs = np.loadtxt('llbb_run2_obs.txt', unpack=True)
    x_llbb_exp, y_llbb_exp = np.loadtxt('llbb_run2_exp.txt', unpack=True)
    graph_llbb_obs1=ROOT.TGraph()
    graph_llbb_obs2=ROOT.TGraph()
    graph_llbb_exp=ROOT.TGraph()
    for i in range(0,len(x_llbb_obs)):
      BRbb=1.0
      BRtt=1.0
      BRmm=1.0
      for j in range(0,len(list_mass)):
         if list_mass[j]<=x_llbb_obs[i]:
            BRbb=list_BRbb[j]
            BRtt=list_BRtt[j]
            BRmm=list_BRmm[j]
      #print(x_llbb_obs[i] , y_llbb_obs[i]/(2*100*BRbb*BRtt + 2*100*BRbb*BRmm) , y_llbb_exp[i]/(2*100*BRbb*BRtt + 2*100*BRbb*BRmm))
      graph_llbb_obs1.SetPoint(i,x_llbb_obs[i],y_llbb_obs[i]/(2*100*BRbb*BRtt + 2*100*BRbb*BRmm))
      graph_llbb_exp.SetPoint(i,x_llbb_exp[i],y_llbb_exp[i]/(2*100*BRbb*BRtt + 2*100*BRbb*BRmm))
    graph_llbb_obs2= graph_llbb_obs1.Clone()
    graph_llbb_obs1.SetPoint(i+1,x_llbb_obs[i],10000000)
    graph_llbb_obs1.SetPoint(i+2,x_llbb_obs[0],10000000)
    for j in range(0,len(list_mass)):
      if list_mass[j]<=x_llbb_obs[0]:
          BRbb=list_BRbb[j]
          BRtt=list_BRtt[j]
          BRmm=list_BRmm[j]
    ...
    graph_mmtt_obs1.SetPoint(i+1,x_mmtt_obs[i],10000000)
    graph_mmtt_obs1.SetPoint(i+2,x_mmtt_obs[0],10000000)
    for j in range(0,len(list_mass)):
    ...
    graph_mmtt_boosted_obs1.SetPoint(i+1,x_mmtt_boosted_obs[i],10000000)
    graph_mmtt_boosted_obs1.SetPoint(i+2,x_mmtt_boosted_obs[0],10000000)
    for j in range(0,len(list_mass)):
    ...
    graph_tttt_obs1.SetPoint(i+1,x_tttt_obs[i],10000000)
    graph_tttt_obs1.SetPoint(i+2,x_tttt_obs[0],10000000)
    for j in range(0,len(list_mass)):
    ...
    graph_ttttv2_obs1.SetPoint(i+1,x_ttttv2_obs[i],10000000)
    graph_ttttv2_obs1.SetPoint(i+2,x_ttttv2_obs[0],10000000)
    for j in range(0,len(list_mass)):
    ...
    for i in range(0,len(x_mmmm_obs)):
      ...
      for j in range(0,len(list_mass)):
         if list_mass[j]<=x_mmmm_obs[i]:
            BRmm=list_BRmm[j]
      #print(x_mmmm_obs[i], BRmm, y_mmmm_obs[i])
      if args.run==2:
    ...
    graph_mmmm_obs1.SetPoint(i+1,x_mmmm_obs[i],10000000)
    graph_mmmm_obs1.SetPoint(i+2,x_mmmm_obs[0],10000000)
    for j in range(0,len(list_mass)):
    ...
    ROOT.gPad.SetTickx()
    ROOT.gPad.SetTicky()
    hr=canv.DrawFrame(1, 0.0001, 62., 10000000.);
    ...
    if (args.model==3):
        hr=canv.DrawFrame(1, 0.00001, 62., 10000.);
    if (args.model==4):
        hr=canv.DrawFrame(1, 0.0000001, 62., 10000000.);
    ...
    hr.SetYTitle("95% CL on B(H#rightarrow aa)");
...
#    hr.GetYaxis().SetRangeUser(0.000001,1000000);
    ...
    graph_llbb_exp.SetFillStyle(3004);
    graph_llbb_exp.SetFillColor(colors['llbb'][0]);
    graph_llbb_exp.SetLineStyle(1);
    if args.run==2:
      graph_llbb_exp.Draw("Csame");
    graph_llbb_obs2.SetLineColor(colors['llbb'][1]);
    graph_llbb_obs2.SetLineStyle(1);
    graph_llbb_obs2.SetLineWidth(1);
    graph_llbb_obs2.SetMarkerStyle(20);
    graph_llbb_obs2.SetMarkerSize(0.7);
    graph_llbb_obs2.SetMarkerColor(colors['llbb'][1]);
    graph_llbb_obs1.SetLineColor(colors['llbb'][1]);
    graph_llbb_obs1.SetFillColor(colors['llbb'][2]);
    graph_llbb_obs1.SetFillStyle(1001); #3005
    if args.run==2:
